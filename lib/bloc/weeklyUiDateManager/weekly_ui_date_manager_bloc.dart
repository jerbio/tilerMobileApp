...
class WeeklyUiDateManagerBloc
    extends Bloc<WeeklyUiDateManagerEvent, WeeklyUiDateManagerState> {
  WeeklyUiDateManagerBloc()
      : super((() {
  ...
  void _onUpdateSelectedWeek(
      UpdateSelectedWeek event, Emitter<WeeklyUiDateManagerState> emit) {
    ...
    if (Utility.isDateWithinPickerRange(event.selectedDate.dayDate) &&
        !listEquals(selectedWeek, state.selectedWeek)) {
      emit(state.copyWith(
  ...
  void _onUpdateTempDate(
      UpdateTempDate event, Emitter<WeeklyUiDateManagerState> emit) {
    if (Utility.isDateWithinPickerRange(event.tempDate))
      emit(state.copyWith(tempDate: event.tempDate));
  }

  void _onSetTempSelectedWeek(
      SetTempSelectedWeek event, Emitter<WeeklyUiDateManagerState> emit) {
    if (Utility.isDateWithinPickerRange(event.selectedDate.dayDate)) {
  ...
  void _onResetTempState(
      ResetTempEvent event, Emitter<WeeklyUiDateManagerState> emit) {
    emit(state.copyWith(
  ...
  void _onLogOutWeeklyUiDateManagerEvent(LogOutWeeklyUiDateManagerEvent event,
      Emitter<WeeklyUiDateManagerState> emit) {
