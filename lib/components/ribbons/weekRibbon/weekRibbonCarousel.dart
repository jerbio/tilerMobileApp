...
class _WeeklyRibbonCarouselState extends State<WeeklyRibbonCarousel> {
  ...
  void initState() {
    ...
    _initialItem=_weeks.indexWhere((week) => week.first.isAtSameMomentAs(state.selectedWeek.first));
  ...
  Widget _buildWeekWidget(List<DateTime> week) {
    return Row(
      ...
      children: week.map((date) => Container(
        child: WeekDayButton(
          dateTime: date,
  ...
  void _trimWeeksOnNextLoad(int selectedItem) {
    ...
    if(nextMonthWeeks[0].first.day!=1)
      startRangeIndex=0;
