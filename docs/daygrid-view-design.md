...
| P6 | Grid body restyle (gutter, hour lines, tile card, pinned card) | 47a93bb | Done | Step 16.1; `tileCardStyle.dart` (pastel tint + `compactTimeRange`), `_TilerEventInnerGridWidget` (accent bar / name / time range, no glyph — C24 revised), gutter cells → `outlineVariant` / `onSurfaceVariant`, pinned card → tertiary + "All day"; `SubCalendarEvent.isAllDay` ms-vs-µs unit fix (now also correctly excludes all-day tiles from `ConflictGroup.detectGroups`); `test/daygrid_tile_card_style_test.dart` (19); on-device verified 2026-09-11. Now-line dot skipped (grey ruler already isolates it). Execution order reordered on-device: 16.3 → 16.4 → 16.5 next, then 16.2 / 16.6 / 16.7 / 16.8 |
| P6 | Tile width/height transitions (closes an existing snap gap) | 59fe664 | Done | Step 16.2; `width`/`height` animate on the same `AnimatedPositioned` as `top`/`left`; card content clipped (`ClipRect` + `OverflowBox`) during size transitions so the target-height caption never overflows a still-growing box; `daygrid_transitions_test` (+1) |
| P6 | `CustomScrollView(center:)` host + negative-extent header slot | bf0c501 | Done | Step 16.3; C18 — grid stays anchored at `pixels == 0`, header lives in `[minScrollExtent, 0)`; zero scroll↔time math changes |
| P6 | `DayGridScrollHeader` (big date, subtitle, compact swipeable day strip, conflict + RSVP banner rows) | bc2dd2a | Done | Step 16.4; C21/C23; built in isolation |
| P6 | Top bar rework: toggle · date pill ▾ · day-summary (any day) · search · settings, cross-fade by reveal progress | f94944b | Done | Step 16.5; C19/C20; `DayGridScope` (shared `DayGridController` + reveal notifier) provided by `GridDailyPageBody`; `DaySummaryHeader` mount / chip strip / `DayRibbonTab` retired from grid mode; `dayGridBannerStrip.dart` → `dayGridAlerts.dart`. **Swipe-repositioning fix folded in:** grid `ScrollController(initialScrollOffset:)` so a fresh day page paints at its first tile hour on frame 1; ONE shared zoom controller (restored once) instead of per-page async restore — zoom is now live across days. On-device verified 2026-09-11 |
| P6 | Travel in-column band tier | (this commit) | Done | Step 16.6; the >=56px tier is a full-column pastel card (glyph · `Travel • N min` · travel window · directions chevron) replacing the gutter pill; gutter hairline/icon tiers unchanged below it; drawn in the travel layer beneath tiles, not an overlap participant; `daygrid_travel_band_test` (+3) |
