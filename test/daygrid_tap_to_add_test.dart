...
// (snapped) time and opens the existing `AddTileEntry` route with a `PreTile`.
//
...
// The widget tests prove the daily grid actually pushes `AddTileEntry` with the
...
/// Enlarges the test surface so the (tall) `AddTileEntry` screen pushed by the
...
void main() {
  ...
  group('DayGridWidget tap-to-add wiring (daily view)', () {
    ...
    testWidgets('empty-area tap pushes AddTileEntry seeded at the tapped time',
        (tester) async {
      _setSurface(tester);
      ...
      await tester.pump();
      // Advance the fake clock past AddTileEntry's 700ms auto-result Timer
      ...
      final addTile = tester.widget<AddTileEntry>(find.byType(AddTileEntry));
    ...
    testWidgets('empty-area tap snaps DOWN to the 15-min interval',
      ...
      await tester.pump();
      // Fire AddTileEntry's 700ms auto-result Timer (see above) so no Timer is
    ...
    testWidgets('no day supplied -> tap-to-add is disabled (read-only)',
        (tester) async {
      final bloc = _RecordingScheduleBloc();
      // No `day` (as in DayCast): an empty-area tap must NOT push AddTileEntry.
      await tester.pumpWidget(_buildApp(
        bloc: bloc,
        tiles: anchorTiles,
        now: now,
      ...
      expect(find.byType(AddTileEntry), findsNothing);
      await _closeBloc(tester, bloc);
    });

    testWidgets(
        'dismissing AddTileEntry before the 700ms auto-result fires leaves no pending Timer',
        (tester) async {
      _setSurface(tester);
      final bloc = _RecordingScheduleBloc();
      await tester.pumpWidget(_buildApp(
      ...
      // Push AddTileEntry; initState schedules the 700ms auto-result Timer because
      // the seeded preTile has location == null.
      await tester.tapAt(const Offset(200, 320));
      await tester.pump();
      // Settle the push so AddTileEntry is built and its initState Timer is
      // scheduled. 100ms keeps the fake clock well under the 700ms
      // auto-result Timer.
      await tester.pump(const Duration(milliseconds: 100));
      expect(find.byType(AddTileEntry), findsOneWidget);

      // Dismiss AddTileEntry while the 700ms auto-result Timer is still pending:
      ...
      await tester.pump(); // frame that detaches & disposes AddTileEntry.
