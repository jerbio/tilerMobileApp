...
// Step 18.2 (C40–C43): the grid paints the segments as 3 px bars in a 6 px
// lane at the far right (right of the travel rail), in a fixed lavender
// (`TileColors.tertiaryContainer`), non-interactive, past half dimmer on
// today, positioned with the tiles' own animation gate.
...
void main() {
  ...
  group('DayGridWidget occupancy rail (C40–C43)', () {
    ...
    testWidgets('today: the past half is dimmer than the future half',
      ...
      // The rail's own fixed colour (C40), not a theme-derived neutral.
      expect(futureColor.withValues(alpha: 1), TileColors.tertiaryContainer);
      expect(pastColor.withValues(alpha: 1), TileColors.tertiaryContainer);
    });
