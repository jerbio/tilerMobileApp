    ...
    if (widget.legendOptions.showLegends) {
      ...
      return Padding(
        ...
        child: Wrap(
          ...
          children: widget.dataMap!.entries
              .map(
                (item) => Legend(
                  title: item.key,
                  duration:toHuman(Duration(milliseconds: item.value.toInt()),abbreviations: true) ,
                  color: isGradientPresent
                      ? getGradient(
                          widget.gradientList!, legendTitles!.indexOf(item.key),
                          isNonGradientElementPresent:
                              isNonGradientElementPresent,
                          emptyColorGradient: widget.emptyColorGradient)[0]
                      : getColor(
                          widget.colorList,
                          legendTitles!.indexOf(item.key),
                        ),
